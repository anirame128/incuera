...
var Incuera = class {
  ...
  init() {
    ...
    this.stopRecording = record({
      ...
      checkoutEveryNms: 15 * 60 * 1e3,
      // Full snapshot every 15 min
      recordCanvas: true,
      collectFonts: true,
      maskAllInputs: false,
      // We'll handle privacy later
      sampling: {
        mousemove: true,
        scroll: 150,
        // Emit scroll events every 150ms
        input: "all"
      }
    });
   ...
   */
  async uploadEvents(events, isUnloading = false) {
    try {
      const headers = {
      ...
      const response = await fetch(`${this.config.apiHost}/api/ingest`, {
        ...
        // Critical for beforeunload, but has 64KB limit so only use when needed
      });
      if (!response.ok) {
...
var index_default = Incuera;
if (typeof window !== "undefined") {
  window.Incuera = Incuera;
}
export {
